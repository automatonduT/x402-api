...
const ritual = (() => { /* RITUAL_SCAN_V2: cherche tout artefact rituel recent, pas un chemin devine */
  try {
    const dirs=[path.join(ROOT,'logs'),path.join(ROOT,'backups'),require('os').homedir()+'/.automaton'];
    const cutoff=Date.now()-24*3600*1000;
    for(const d of dirs){
      let names=[]; try{names=fs.readdirSync(d)}catch(e){continue}
      for(const nme of names){
        if(!/^ritual/i.test(nme))continue;
        const p=path.join(d,nme);
        try{ if(fs.statSync(p).mtimeMs<cutoff)continue;
          if(fs.readFileSync(p,'utf8').includes('RITUAL_OK'))return 'RITUAL_OK ('+nme+')';
        }catch(e){}
      }
    }
    return 'non tourne (<24h)';
  } catch(e){ return 'erreur scan'; }
})();
